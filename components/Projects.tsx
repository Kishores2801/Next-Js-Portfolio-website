...
export default function Projects() {
  ...
  // Extract unique categories from project tags to display as filter buttons
  ...
  return (
    <div className="flex flex-col relative min-h-screen text-center md:text-left max-w-9xl px-3 sm:px-6 md:px-8 justify-evenly mx-auto items-center overflow-hidden">
      <h3 className="absolute top-40 uppercase tracking-[6px] sm:tracking-[8px] text-blue-500 text-lg sm:text-xl md:text-2xl">
        Project Showcase
      </h3>

      <div className="mt-6 flex flex-col items-center justify-center w-full">
        {/* Filter buttons */}
        <div className="flex justify-center rounded-md items-center flex-wrap mb-6 gap-2 px-2">
          {uniqueCategories.length > 0 && (
            <button
              onClick={() => handleWorkFilter('All')}
              className={`px-2 py-1 sm:px-3 sm:py-2 cursor-pointer rounded-xl transition-all duration-300 ${
        ...
        {/* Work items with scrollable container */}
        
        <motion.div
          animate={animateCard}
          transition={{ duration: 0.5, delayChildren: 0.5 }}
          className="flex flex-wrap justify-center gap-4 p-2 w-full max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200"
        >
          {filterWork.map((work, index) => (
            <div
              key={index}
              className="w-40 sm:w-52 md:w-60 bg-white rounded-xl shadow-md border border-gray-200 hover:shadow-lg transition-transform transform hover:-translate-y-1 text-center cursor-pointer overflow-hidden"
            >
              ...
              <div className="p-2 sm:p-3">
                <h4 className="text-sm sm:text-lg font-semibold text-gray-800 mb-1">{work.title}</h4>
                <p className="text-xs sm:text-sm text-gray-600">{work.description}</p>
                <div className="mt-2 flex flex-wrap justify-center gap-2">
                  {work.skills && work.skills.map((skill, skillIndex) => (
                    <img
                      key={skillIndex}
